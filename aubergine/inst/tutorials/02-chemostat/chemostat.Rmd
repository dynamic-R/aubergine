...
chemostat <- function(time, init, parms) {
  with(as.list(c(init, parms)), {
    r       <- r_max * N/(k + N)  # Monod equation
    dN_dt   <- D * (N0 - N) - 1/Y * r * P
    dP_dt   <- r * P - D * P
    list(c(dN_dt, dP_dt), r = r)
   })
}
parms <- c(
  r_max = 0.5,    # 1/d
  k     = 0.5,    # half saturation constant, P (mol/m3)
...
chemostat <- function(time, init, parms) {
  with(as.list(c(init, parms)), {
    ...
    dN_dt  <- D * (N0 - N) - r * P * 1/Y
    dP_dt  <- r * P - D * P - r_z * Z * 1/Y_z
    dZ_dt  <- r_z * Z - D * Z
