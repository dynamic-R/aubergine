...
chemostat <- function(time, init, parms) {
  with(as.list(c(init, parms)), {
    p_growth <- p_growmax * N / (km_pgrowth + N) * P
    n_import <- d * N0
    n_export <- d * N
    p_export <- d * P
...
parms <- c(
  p_growmax = 0.5,    # 1/d
  km_pgrowth     = 0.5,    # half saturation constant, P (mol/m3)
  c_pn     = 1/106,    # yield = 1/stoichiometric conversion from N to P (C:P ratio)
  d     = 0.1,    # dilution rate 1/d
  N0    = 5       # P in inflow (mol/m3)
)
times <- seq(0, 40, 0.1)        # (d)
init  <- c(N = 5, P = 10)    # Phytoplankton C and Phosphorus P (mol/m3)
```
...
chemostat <- function(time, init, parms) {
  with(as.list(c(init, parms)), {
    ...
    p_growth <- p_growmax * N / (km_pgrowth + N) * P
    z_growth <- z_growmax * P / (km_zgrowth + P) * Z
    ...
    dN_dt   <- n_import - c_pn * p_growth - n_export
    dP_dt   <- p_growth - c_zp * z_growth - p_export
...
parms <- c(
  p_growmax  = 0.5,   # max. phytoplankton growth rate 1/d
  z_growmax  = 0.2,   # max. zooplankton growth rate 1/d

  km_pgrowth = 0.5,   # half sat. constant of phyto growth on nutrient (mol/m3)
  km_zgrowth = 100,   # half stat constant of zoo growth on phyto (mol/m3)

  c_pn       = 1/106, # nutrient demand of phyto (Redfield phosphorus : carbon)
  c_zp       = 10,    # food demand of zoo (carbon, 10:1)
