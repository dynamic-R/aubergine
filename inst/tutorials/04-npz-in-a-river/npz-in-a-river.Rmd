...
parms<- c(r_pgrow=0.5, km_n=1.0, c_pn=1/106, d=0.1, N0=5)
y    <- c(N = 5, P = 1)  
...
parms <- c(
  ...
  km_n    = 0.5,   # Monod ct of phyto growth on nutrient (mmol/m3)
  km_p    = 100,   # Monod ct of zoo grazing on phyto (mmol/m3)
...
parms<- c(r_pgrow=0.5, r_zgraz=0.4, r_zloss=0.01, km_n=0.5, 
          km_p=100, c_pn=1/106, asseff=0.3, d=0.11, N0=500)
y   <- c(N=5, P=1, Z=1)  
...
riverNPZ <- function(time, y, parms) {
  with(as.list(parms), {
    ...
    p_growth  <- r_pgrow * N / (km_n + N) * P
    z_grazing <- r_zgraz * P / (km_p + P) * Z              ## ADDED ##
