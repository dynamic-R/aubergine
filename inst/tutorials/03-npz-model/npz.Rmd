...
the behaviour of this more complex model.
...
\begin{align}
\frac{dZ}{dt} &= c_{asseff} \cdot c_{pn} \cdot r_{zgraz} \cdot \frac{P}{km_P + P} - r_{zloss} \cdot Z - d \cdot Z
\end{align}

with $c_{asseff}$ = assimilation efficiency, $r_{zgraz}$ = zooplankton grazing 
parameter, $r_{zloss}$ = zooplankton loss parameter, and $km_P$ = Monod constant 
...
\begin{align}
    p_{growth}    &= r_{pgrow} \cdot \frac{N}{km_n + N}  \cdot P\\
    z_{grazing}   &= r_{zgraz}  \cdot \frac{P}{km_p + P} \cdot Z\\
...
```{r chemostat-npz, exercise=TRUE, exercise.lines = 25}
