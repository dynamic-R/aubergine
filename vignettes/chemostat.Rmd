...
vignette: >
  %\VignetteIndexEntry{Modelling in R: A Chemostat}
...
```{r, chemostat-ex1, eval=FALSE, fig.height=3, fig.width=8}
<<chemostat>>
times <- seq(from=0, to=40, length.out=100)  # simulation time (d)
...
```{r, chemostat-ex2, eval=TRUE, fig.height=3, fig.width=8}
<<chemostat>>
test_parms <- parms
test_parms["d"] <- 0.1
times <- seq(from=0, to=40, length.out=100)  # simulation time (d)
...
```{r, steadystate, eval=FALSE, fig.height=3, fig.width=8}
<<exercise>>
state <- data.frame(
...
```{r, steadystate-hint, eval=FALSE, fig.height=3, fig.width=8}
