---
title: "03. Modelling in R: From a Chemostat to NPZ with Phyto- and Zooplankton"
...
vignette: >
  %\VignetteIndexEntry{03. Modelling in R: From a Chemostat to NPZ with Phyto- and Zooplankton}
...
Putting things together, you will get a code similar to the following. There are
of course always different possibilities, just compare your solution with this.
...
```{r npz-scenario-exercise, eval=FALSE}
<<chemostat-npz-code>>
times <- seq(0, 800)
parms["d"] <- 0.0
...
## Dynamic behavior: Hints and solution

If we try the following, we can observe different qualitative system behavior. 
The individual scenarios allow to reproduce the simulations yourself. Plot the
scenarios and try to understand whether it is an equilibrium and which pupulations
survive or die out.

### Scenario 0: Batch

```{r npz-scenario0, fig.height=2, fig.width=7, fig.keep="none"}
times <- seq(0, 800)
parms["d"] <- 0.0
out0 <- ode(y, times, npz, parms)
plot(out0, mfrow = c(1, 3))
...
```{r npz-scenario1, fig.height=2, fig.width=7, fig.keep="none"}
parms["d"] <- 0.2
...
```{r npz-scenario2, fig.height=2, fig.width=7, fig.keep="none"}
parms["d"] <- 0.07
...
```{r npz-scenario3, fig.height=2, fig.width=7, fig.keep="none"}
parms["d"] <- 0.09
...
```{r npz-scenario4, fig.height=2, fig.width=7, fig.keep="none"}
parms["d"] <- 0.5
...
```{r npz-scenario5, fig.height=2, fig.width=7, fig.keep="none"}
parms["d"] <- 0.081
...
The final plot shows all scenarios together.

```{r npz-all, fig.height=3, fig.width=7}
